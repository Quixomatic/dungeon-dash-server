...
export class BSPDungeonGenerator {
  ...
  generate() {
    ...
    const dungeonX = Math.floor((this.config.worldTileSize - this.config.dungeonTileSize) / 2);
    const dungeonY = Math.floor((this.config.worldTileSize - this.config.dungeonTileSize) / 2);
    
    // Create root leaf for the dungeon area
    const rootLeaf = new BSPLeaf(
      dungeonX, dungeonY,
      this.config.dungeonTileSize, this.config.dungeonTileSize
    );
  ...
  createSpawnRooms() {
    ...
    const spawnRadius = Math.floor(this.config.dungeonTileSize / 2) + this.config.spawnRoomDistance;
    
    console.log(`Creating ${spawnCount} spawn rooms at radius ${spawnRadius} tiles from center (${dungeonCenterX}, ${dungeonCenterY})`);
    ...
    for (let i = 0; i < spawnCount; i++) {
      ...
      console.log(`Spawn room ${i} at position (${roomX}, ${roomY}) with size ${spawnRoomSize}x${spawnRoomSize}`);
